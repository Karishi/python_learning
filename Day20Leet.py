...
def findExteriorWater(matrixList):
    ...
    if 0 in matrixList[0]:
        for node in range(len(matrixList)):
            if matrixList[0][node] == 0:
                processList.append(waterNode(node, 0))
                seen.append((node, 0))
    for space in range(1, len(matrixList)-1):
        ...
        if matrixList[space][-1] == 0:
            ...
            seen.append((len(matrixList[space])-1, space))
    if 0 in matrixList[-1]:
        for node in range(len(matrixList)):
            if matrixList[-1][node] == 0:
                processList.append(waterNode(node, len(matrixList[space])-1))
                seen.append((node, len(matrixList[space])-1))


    while len(processList) > 0:
        waterTotal += 1
        current = processList.pop(0)
        x, y = current.x, current.y
        # Check East
        if x < len(matrixList[y])-1 and matrixList[y][x+1] == 0:
            if (x+1, y) not in seen:
                seen.append((x+1, y))
                node = waterNode(x+1, y)
                processList.append(node)
        # Check South
        if y < len(matrixList)-1 and matrixList[y+1][x] == 0:
            if (x, y+1) not in seen:
                seen.append((x, y+1))
                node = waterNode(x, y+1)
                processList.append(node)
        # Check West
        if x > 0 and matrixList[y][x-1] == 0:
            if (x-1, y) not in seen:
                seen.append((x-1, y))
                node = waterNode(x-1, y)
                processList.append(node)
        # Check North
        if y > 0 and matrixList[y-1][x] == 0:
            if (x, y-1) not in seen:
                seen.append((x, y-1))
                node = waterNode(x, y-1)
                processList.append(node)
